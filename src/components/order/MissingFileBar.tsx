...
export default function MissingFileBar({ orderId, missing, editable }: MissingFileBarProps) {
  ...
  /*
    ★ 폰에서 찍은 사진은 PC 에서 다시 고를 수 없습니다 — 파일이 폰에
      ...
      보내므로(PhotoQueueBar), 그렇게 말해 줍니다. (2026-09-07)
  */
  ...
  return (
    <div className="mb-2 rounded-md border border-[#F3C6C6] bg-[#FDECEA] px-[11px] py-[9px]">
      ...
      {/* ★ 문구만 부드럽게 (사용자 결정 2026-09-07 — 색과 모양은 그대로). 첫 마디가 "주문은 그대로" */}
      <p className="text-[12.5px] font-bold leading-relaxed text-[#C4383A]">
        ⚠ 파일 {missing.length}개가 아직 안 올라왔습니다. 주문은 그대로 있으니 파일만 이어서 올리면 됩니다.
      </p>
      {phonePhotos > 0 && (
        <p className="mt-0.5 text-[12px] leading-relaxed text-[#8C4A48]">
          폰에서 찍은 사진은 폰의 덴플로우를 다시 열면 이어서 올라갑니다.
        </p>
      )}
      ...
      {editable ? (
        <>
          ...
          <p className="mt-1 text-[11px] text-[#A8706E]">같은 파일을 다시 고르면 그 자리에 들어갑니다.</p>
        </>
      ) : (
        <p className="mt-1.5 text-[11px] text-[#A8706E]">치과가 이어서 올리면 채워집니다.</p>
      )}
