...
/**
 * 한 번에 몇 개까지 동시에 올리는가 (작업지시서 §3-2).
 *
 * ★ 하나씩이면 왕복 시간이 그대로 쌓이고, 다 던지면 회선이 막혀
 *   전부 느려지고 끊기기도 쉽습니다.
 *
 * ★ 3 → **4** (2026-08-21 실측). 10MB 짜리 열 개를 올려 봤습니다:
 *
 *       동시 1개   7.1초
 *       동시 3개   4.2초
 *       동시 5개   3.5초
 *       동시 8개   3.3초   (5 위로는 거의 안 줄어듦)
 *
 *   5가 제일 빨랐지만 4로 둡니다. 치과 회선은 올리는 쪽이 얇아
 *   숫자를 늘려도 총량이 안 늘고, 끊길 때 **한꺼번에 여러 개**가
 *   같이 넘어집니다. 4면 이득의 절반쯤을 가져오면서 그 위험이 얕습니다.
 */
