...
/**
 * 이어올리기 조각 크기. **6MB 의 배수여야 합니다.**
 *
 * ★ 6 → **24MB** (사용자 요청 2026-08-21 — "업로드 속도를 올리고 싶다").
 *
 *   전에는 6MB 로 보냈습니다. Supabase 문서가 tus 클라이언트에
 *   "6MB 로 두고 바꾸지 말라" 고 적어 둔 것을 그대로 따랐습니다.
 *   그 말은 **최소 크기**(S3 조각 하한)를 가리키는 것이었고, 배수로
 *   키우는 것은 막지 않았습니다.
 *
 * ★★ 운영 저장소에 **진짜 파일로 재 봤습니다** (150MB, 2026-08-21):
 *
 *        6MB 조각   25회 왕복   15.4초
 *       24MB 조각    7회 왕복   10.2초   ← 여기
 *       36MB 조각    5회 왕복   10.0초   (더 키워도 안 빨라짐)
 *
 *   왕복 한 번마다 조각을 받아 S3 에 붙이고 답을 주는 시간이 있습니다.
 *   그 횟수가 25번에서 7번으로 줄어드는 것이 전부입니다. **3분의 1**이
 *   줄었습니다.
 *
 * ★ 왜 무한정 안 키우나. 끊기면 **그 조각을 통째로 다시** 보냅니다.
 *   24MB 면 최악이 24MB 재전송입니다. 36MB 부터는 빨라지지도 않으면서
 *   다시 보낼 양만 늘어납니다.
 *
 * ★ 바꾸려거든 6의 배수로만. 그리고 값만 고치고 됐다고 하지 마세요 —
 *   실제 크기로 올려 봐야 압니다 (버킷 상한 때 데인 그것과 같습니다).
 */
export const TUS_CHUNK_BYTES = 24 * 1024 * 1024;
